...
S_default = 4100000
known_infections = 100 # update daily
...
doubling_time = st.sidebar.number_input(
    "Doubling time before social distancing (days)", value=doubling_time_input, step=1, format="%i"
)
...
if st.button('export data'):
    projection_admits.to_csv("/app/data/walrus_pancake.csv")
    wd = getcwd()
    st.markdown("current working director is: {wd}".format(wd=wd))
